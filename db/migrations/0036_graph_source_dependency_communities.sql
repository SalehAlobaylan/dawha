...
ALTER TABLE research_runs
  ...
  CHECK (graph_operation IS NULL OR graph_operation IN ('common_ancestor_path', 'evidence_connection', 'branch_claims', 'source_entities', 'geographic_path', 'shortest_relationship_path', 'connected_component', 'relationship_impact', 'branch_structure_comparison', 'ancestor_frontier', 'source_dependency_neighborhood', 'source_dependency_communities'));
...
ALTER TABLE research_graph_paths
  ...
  CHECK (operation IN ('common_ancestor_path', 'evidence_connection', 'branch_claims', 'source_entities', 'geographic_path', 'shortest_relationship_path', 'connected_component', 'relationship_impact', 'branch_structure_comparison', 'ancestor_frontier', 'source_dependency_neighborhood', 'source_dependency_communities'));

CREATE TABLE research_graph_source_dependency_communities (
  ...
  CHECK (char_length(partition_fingerprint) BETWEEN 1 AND 128),
...
CREATE INDEX research_graph_source_dependency_communities_root_idx
  ON research_graph_source_dependency_communities (root_source_id, created_at DESC);
