...
import { ArrowLeft, CalendarDays, ChevronLeft, CircleAlert, Layers3, MapPinned, Plus, Route, Search, SlidersHorizontal } from "lucide-react";
...
export function PlacesPage() {
  ...
  const [period, setPeriod] = useState("كل الفترات");
  ...
  return (
    <div className="page-stack">
      ...
      <section className="places-toolbar"><div className="place-breadcrumb"><MapPinned size={16} /><span>فهرس المواضع</span><ChevronLeft size={14} /><strong>الرياض وما حوله</strong></div><div className="place-toolbar-actions"><label className="place-period"><CalendarDays size={15} /><select value={period} onChange={(event) => setPeriod(event.target.value)}><option>كل الفترات</option><option>قبل ١١٥٠هـ</option><option>١١٥٠ — ١٢٥٠هـ</option><option>بعد ١٢٥٠هـ</option></select></label><button className="secondary-button" type="button"><SlidersHorizontal size={15} /> تصفية</button><button className="primary-button" type="button"><Plus size={15} /> إضافة موضع</button></div></section>
      ...
      <section className="places-index-section"><div className="places-index-head"><div><div className="eyebrow">قائمة المواضع</div><h2>ابنِ زمنك المفتوح</h2><p>اختر موضعاً، أو ابحث عن موضع قديم لا تزال تسميته غير مستقرة.</p></div><label className="library-search"><Search size={15} /><input placeholder="ابحث عن موضع" /></label></div><div className="place-index-grid">{places.map((place) => <button type="button" className="place-index-card" key={place.id} onClick={() => setSelected(place)}><span className={`place-index-icon place-icon-${place.status === "موثق" ? "source" : place.status === "متنازع عليه" ? "disputed" : "interpretation"}`}><MapPinned size={17} /></span><span className="place-index-copy"><strong>{place.name}</strong><small>{place.type} · {place.period}</small></span><span className="place-index-count">{place.evidence} إشارات</span></button>)}</div></section>
